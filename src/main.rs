...
const SQL_ALLOWLIST_PATTERN: &str = r#"(?i)^SELECT\s+[a-zA-Z0-9_,\s*\(\)\.\/:]+\s+FROM\s+[a-zA-Z0-9_]+(?:\s+WHERE\s+[a-zA-Z0-9_()=<>\'\s%\*\+\-\./@\?:\$\[\]\"{}]+)?(?:\s+GROUP BY\s+[a-zA-Z0-9_,\s\(\)\.\/:]+)?(?:\s+ORDER BY\s+[a-zA-Z0-9_,\s\(\)\*]+(?:ASC|DESC)?)?(?:\s+LIMIT\s+\d+)?$"#;
